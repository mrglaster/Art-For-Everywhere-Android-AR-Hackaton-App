...
inline ArInstantPlacementPoint*
...
#define AR_DEPRECATED(_deprecation_string) __attribute__((deprecated(_deprecation_string)))
#else
...
AR_DEFINE_ENUM(ArTrackableType){ /// The base Trackable type. Can be passed to @c ::ArSession_getAllTrackables
...
AR_DEFINE_ENUM(ArSessionFeature){
    ...
    AR_SESSION_FEATURE_FRONT_CAMERA AR_DEPRECATED("To create a session using the front-facing (selfie) camera, use "
...
AR_DEFINE_ENUM(ArStatus){ /// The operation was successful.
                          AR_SUCCESS = 0,
...
/// @c ::ArCamera.
AR_DEFINE_ENUM(ArTrackingState){ /// The object is currently tracked and its pose is current.
...
AR_DEFINE_ENUM(ArTrackingFailureReason){ /// Indicates expected motion tracking behavior. Always returned when
...
AR_DEFINE_ENUM(ArCloudAnchorState){
    ...
    AR_CLOUD_ANCHOR_STATE_ERROR_SERVICE_UNAVAILABLE AR_DEPRECATED("AR_CLOUD_ANCHOR_STATE_ERROR_SERVICE_UNAVAILABLE is deprecated in "
...
AR_DEFINE_ENUM(ArAvailability){ /// An internal error occurred while determining ARCore availability.
...
AR_DEFINE_ENUM(ArInstallStatus){ /// The requested resource is already installed.
...
AR_DEFINE_ENUM(ArInstallBehavior){ /// Hide the Cancel button during initial prompt and prevent user from
...
AR_DEFINE_ENUM(ArInstallUserMessageType){ /// Display a localized message like "This application requires ARCore...".
...
AR_DEFINE_ENUM(ArAugmentedImageTrackingMethod){ /// The Augmented Image is not currently being tracked.
...
AR_DEFINE_ENUM(ArPlaneType){ /// A horizontal plane facing upward (for example a floor or tabletop).
...
AR_DEFINE_ENUM(ArLightEstimateState){ /// The @c ::ArLightEstimate is not valid this frame and should not be used
                                      /// for rendering.
...
AR_DEFINE_ENUM(ArPointOrientationMode){ /// The orientation of the @c ::ArPoint is initialized to identity but may
...
AR_DEFINE_ENUM(ArCameraConfigFacingDirection){ /// Camera looks out the back of the device (away from the user).
...
ArStatus ArCoreApk_requestInstall(void* env, void* application_activity, int32_t user_requested_install,
...
/// - @c #AR_UNAVAILABLE_USER_DECLINED_INSTALLATION if the user previously
///   declined installation.
ArStatus ArCoreApk_requestInstallCustom(void* env, void* application_activity, int32_t user_requested_install,
                                        ArInstallBehavior install_behavior, ArInstallUserMessageType message_type,
                                        ArInstallStatus* out_install_status);
...
void ArCameraConfig_getImageDimensions(const ArSession* session, const ArCameraConfig* camera_config, int32_t* out_width,
                                       int32_t* out_height);

/// @ingroup ArCameraConfig
/// Obtains the GPU texture dimensions for the given camera config.
void ArCameraConfig_getTextureDimensions(const ArSession* session, const ArCameraConfig* camera_config, int32_t* out_width,
                                         int32_t* out_height);
...
void ArCameraConfig_getFacingDirection(const ArSession* session, const ArCameraConfig* camera_config,
...
// TODO(b/166280987) Finalize documentation
void ArCameraConfig_getStereoCameraUsage(const ArSession* session, const ArCameraConfig* camera_config,
...
void ArCameraConfigFilter_setTargetFps(const ArSession* session, ArCameraConfigFilter* filter, const uint32_t fps_filters);
...
///     together
// TODO(b/166280987) Finalize documentation
void ArCameraConfigFilter_setStereoCameraUsage(const ArSession* session, ArCameraConfigFilter* filter,
...
void ArCameraConfigFilter_setFacingDirection(const ArSession* session, ArCameraConfigFilter* filter,
...
void ArCameraConfigFilter_getFacingDirection(const ArSession* session, ArCameraConfigFilter* filter,
...
void ArRecordingConfig_getMp4DatasetFilePath(const ArSession* session, const ArRecordingConfig* config, char** out_mp4_dataset_file_path)
    AR_DEPRECATED("Deprecated in release 1.26.0. Please see function documentation");
...
ArStatus ArSession_checkSupported(const ArSession* session, const ArConfig* config)
    AR_DEPRECATED("Deprecated in release 1.2.0. Please see function documentation");
...
ArStatus ArSession_estimateFeatureMapQualityForHosting(const ArSession* session, const ArPose* pose,
...
/// - @c #AR_ERROR_RESOURCE_EXHAUSTED
/// - @c #AR_ERROR_ANCHOR_NOT_SUPPORTED_FOR_HOSTING
ArStatus ArSession_hostAndAcquireNewCloudAnchorWithTtl(ArSession* session, const ArAnchor* anchor, int32_t ttl_days,
...
void ArSession_getSupportedCameraConfigs(const ArSession* session, ArCameraConfigList* list)
    AR_DEPRECATED("Deprecated in release 1.11.0. Please see function documentation.");
...
ArStatus ArFrame_recordTrackData(ArSession* session, const ArFrame* frame, const uint8_t* track_id_uuid_16, const void* payload,
                                 size_t payload_size);
...
void ArCamera_getTrackingFailureReason(const ArSession* session, const ArCamera* camera,
...
void ArCamera_getImageIntrinsics(const ArSession* session, const ArCamera* camera, ArCameraIntrinsics* out_camera_intrinsics);
...
/// @param[in]    camera                 The session's camera.
/// @param[inout] out_camera_intrinsics  The @c ::ArCameraIntrinsics data.
void ArCamera_getTextureIntrinsics(const ArSession* session, const ArCamera* camera, ArCameraIntrinsics* out_camera_intrinsics);
...
void ArCameraIntrinsics_getImageDimensions(const ArSession* session, const ArCameraIntrinsics* intrinsics, int32_t* out_width,
                                           int32_t* out_height);
...
void ArFrame_transformDisplayUvCoords(const ArSession* session, const ArFrame* frame, int32_t num_elements, const float* uvs_in,
                                      float* uvs_out) AR_DEPRECATED("Deprecated in release 1.7.0. Please see function documentation.");
...
void ArFrame_transformCoordinates2d(const ArSession* session, const ArFrame* frame, ArCoordinates2dType input_coordinates,
                                    int32_t number_of_vertices, const float* vertices_2d, ArCoordinates2dType output_coordinates,
...
void ArFrame_hitTestInstantPlacement(const ArSession* session, const ArFrame* frame, float pixel_x, float pixel_y,
                                     float approximate_distance_meters, ArHitResultList* hit_result_list);
...
///     previously allocated using @c ::ArHitResultList_create.  If the list has
///     been previously used, it will first be cleared.
void ArFrame_hitTestRay(const ArSession* session, const ArFrame* frame, const float* ray_origin_3, const float* ray_direction_3,
...
void ArFrame_getUpdatedTrackables(const ArSession* session, const ArFrame* frame, ArTrackableType filter_type,
...
/// used, the list will first be cleared
void ArFrame_getUpdatedTrackData(const ArSession* session, const ArFrame* frame, const uint8_t* track_id_uuid_16,
...
void ArTrackDataList_acquireItem(const ArSession* session, const ArTrackDataList* track_data_list, int32_t index,
...
void ArImage_getPlaneData(const ArSession* session, const ArImage* image, int32_t plane_index, const uint8_t** out_data,
...
void ArLightEstimate_getState(const ArSession* session, const ArLightEstimate* light_estimate,
...
///   y, z].
void ArLightEstimate_getEnvironmentalHdrMainLightDirection(const ArSession* session, const ArLightEstimate* light_estimate,
...
void ArLightEstimate_getEnvironmentalHdrMainLightIntensity(const ArSession* session, const ArLightEstimate* light_estimate,
...
void ArLightEstimate_getEnvironmentalHdrAmbientSphericalHarmonics(const ArSession* session, const ArLightEstimate* light_estimate,
...
void ArLightEstimate_acquireEnvironmentalHdrCubemap(const ArSession* session, const ArLightEstimate* light_estimate,
...
void ArTrackableList_acquireItem(const ArSession* session, const ArTrackableList* trackable_list, int32_t index,
...
void ArInstantPlacementPoint_getTrackingMethod(const ArSession* session, const ArInstantPlacementPoint* instant_placement_point,
...
void ArAugmentedImage_getTrackingMethod(const ArSession* session, const ArAugmentedImage* image,
...
void ArAugmentedFace_getMeshVertices(const ArSession* session, const ArAugmentedFace* face, const float** out_vertices,
...
void ArAugmentedFace_getMeshNormals(const ArSession* session, const ArAugmentedFace* face, const float** out_normals,
...
void ArAugmentedFace_getMeshTextureCoordinates(const ArSession* session, const ArAugmentedFace* face, const float** out_texture_coordinates,
...
void ArAugmentedFace_getMeshTriangleIndices(const ArSession* session, const ArAugmentedFace* face, const uint16_t** out_triangle_indices,
...
///     @c #AR_TRACKING_STATE_PAUSED.
void ArAugmentedFace_getRegionPose(const ArSession* session, const ArAugmentedFace* face, const ArAugmentedFaceRegionType region_type,
                                   ArPose* out_pose);
...
ArStatus ArAugmentedImageDatabase_deserialize(const ArSession* session, const uint8_t* database_raw_bytes, int64_t database_raw_bytes_size,
                                              ArAugmentedImageDatabase** out_augmented_image_database);
...
void ArAugmentedImageDatabase_serialize(const ArSession* session, const ArAugmentedImageDatabase* augmented_image_database,
                                        uint8_t** out_image_database_raw_bytes, int64_t* out_image_database_raw_bytes_size);
...
ArStatus ArAugmentedImageDatabase_addImage(const ArSession* session, ArAugmentedImageDatabase* augmented_image_database,
                                           const char* image_name, const uint8_t* image_grayscale_pixels, int32_t image_width_in_pixels,
                                           int32_t image_height_in_pixels, int32_t image_stride_in_pixels, int32_t* out_index);
...
ArStatus ArAugmentedImageDatabase_addImageWithPhysicalSize(const ArSession* session, ArAugmentedImageDatabase* augmented_image_database,
...
void ArAugmentedImageDatabase_getNumImages(const ArSession* session, const ArAugmentedImageDatabase* augmented_image_database,
...
void ArImageMetadata_getAllKeys(const ArSession* session, const ArImageMetadata* image_metadata, int32_t* out_number_of_tags,
...
// TODO(b/161001774) Finalize documentation
ArStatus ArImageMetadata_getConstEntry(const ArSession* session, const ArImageMetadata* image_metadata, uint32_t tag,
